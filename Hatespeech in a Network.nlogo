...
turtles-own
[
  self-censor-willingness
  confidence
  confidenceValue ;; c Dach
  silent?
  didUpdate ;; variable to track confidence changes. serves as end condition
  hater?
  bannedOrLoggedoff?
]

to check-links
  let normalLinkCount 0
  let haterLinkCount 0
  ask turtles[
    ifelse hater? = TRUE
      [set haterLinkCount haterLinkCount + count link-neighbors]
      [set normalLinkCount normalLinkCount + count link-neighbors]
  ]
  let normalDegree (normalLinkCount / (number-of-nodes - hater-amount))
  let haterDegree (haterLinkCount / hater-amount)
  show "nonHater degree"
  show normalDegree
  show "expected hater degree"
  show average-node-degree * 1.3
  show "hater degree"
  show haterDegree
  show "--------"
end
...
to setup
  clear-all
  if fixed-random-seed? [ random-seed seed ]
  setup-nodes
  ask n-of hater-amount turtles
    [set hater? true
     set self-censor-willingness random-float 0.5
     set confidence 3 + random-float 0.5
    ]
...
to setup-nodes
  ...
  [
    ...
    set confidenceValue random-float 1
    set confidence 1
    set hater? false
    set bannedOrLoggedoff? false
    become-silent
  ]
end

to setup-spatially-clustered-network
  let num-links (average-node-degree * 1.3 * hater-amount)
  while [count links < num-links ]
  [
    ask one-of turtles with [hater? = true]
    [
  ...
  set num-links num-links + (average-node-degree * (number-of-nodes - hater-amount)) / 2
  while [count links < num-links]
  [
    ask one-of turtles with [hater? = false]
    [
      let choice (min-one-of (other turtles with [not link-neighbor? myself and hater? = false])
                   [distance myself])
...
to go
  if all? turtles [didUpdate > 2] [stop
    profiler:stop
  print profiler:report]
  ask turtles with [bannedOrLoggedoff? = FALSE]
  [
    update-confidence
    become-silent
    update-color
...
to-report hater-neighbor
  if any? link-neighbors with [hater? = TRUE and silent? = FALSE and bannedOrLoggedoff? = FALSE]
    [if hater? = FALSE
      [report TRUE]
    ]
  report FALSE
...
to become-silent
  ...
  ifelse self-censor-willingness > confidence
    ...
    [if bannedOrLoggedoff? != TRUE
      [set silent? FALSE]
    ]
  ifelse temp != silent? and bannedOrLoggedoff? = FALSE
    [set didUpdate 0]
    [set didUpdate didUpdate + 1]
end

to update-confidence
  let ownOpinion? hater?
  let nO count (link-neighbors with [hater? != ownOpinion? and silent? = false])
  let nS count (link-neighbors with [hater? = ownOpinion? and silent? = false])
  ifelse ownOpinion? = TRUE
    [set nS nS * 8] ; changed from 5 to 8 during verification
    [set nO nO * 8] ; changed from 5 to 8 during verification
  let delta 0
  ifelse nS + nO = 0
    [set delta 0]
    [set delta (nS - nO)/(nS + nO)]
  set confidenceValue max list (confidenceValue + delta) 0
  ifelse confidence < 0
    [set confidence 0]
    [set confidence 2 * (1 / (1 + exp(0 - confidenceValue))) - 1]
end

to update-color
  if bannedOrLoggedoff? = FALSE
    [ifelse silent?
    [set color gray]
    [ifelse hater? = FALSE
      [set color blue]
...
to counter-measures
  ...
  (ifelse
    choice < 1 - (report-chance / 100) - (logout-chance / 100) - (counterspeech-chance / 100) [
      ;; do nothing
    ]
    choice >= 1 - (report-chance / 100) - (logout-chance / 100) - (counterspeech-chance / 100) and choice < 1 - (logout-chance / 100) - (counterspeech-chance / 100) [
      if random-float 1 < 0.05
      [ask link-neighbors with [hater? = TRUE and silent? = FALSE and bannedOrLoggedoff? = FALSE]
                                               [set bannedOrLoggedoff? TRUE
                                                set didUpdate 3
                                                become-silent
                                                set color [189 100 100]
                                                ;show "Banned"
      ]]
      ;; report
    ]
    choice >= 1 - (logout-chance / 100) - (counterspeech-chance / 100) and choice < 1 - (counterspeech-chance / 100) [
      if any? link-neighbors with [hater? = TRUE and silent? = FALSE and bannedOrLoggedoff? = FALSE]
      [set bannedOrLoggedoff? TRUE
       set didUpdate 3
       become-silent
       set color [134 176 213]
      ]
      ;; logout
    ]
    choice >= 1 - (counterspeech-chance / 100)
    [ ask link-neighbors with [hater? = TRUE and silent? = FALSE and bannedOrLoggedoff? = FALSE]
                                              [update-confidence
                                               become-silent
                                               update-color
...
"" ""
PENS
"Non-hater" 1.0 0 -13345367 true "" "plot (count turtles with [hater? = false and (silent? or bannedOrLoggedoff?)]) / (count turtles with [hater? = false]) * 100"
"Hater" 1.0 0 -2674135 true "" "plot (count turtles with [hater? = true and silent?]) / (count turtles with [hater? = true]) * 100"
"Silenced" 1.0 0 -7500403 true "" "plot (count turtles with [silent?]) / (count turtles) * 100"
...
"" ""
PENS
"Opiniion = +1" 1.0 0 -13345367 true "" "plot (count turtles with [hater? = false and not silent? and not bannedOrLoggedoff?]) / (count turtles with [not silent?]) * 100"
"Opinion = -1" 1.0 0 -2674135 true "" "plot (count turtles with [hater? = true and not silent? and not bannedOrLoggedoff?]) / (count turtles with [not silent?]) * 100"
